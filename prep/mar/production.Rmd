...
Due to weird data reporting for Salmon and Trout, we need to fix Maine's finfish. The `maine_finfish.csv` dataset is used to replace all finfish from Maine. We also use the 2010 data to gapfill forward since the Department of Marine Resources does not report production data after 2010 due to confidentiality statutes ([see their statment here](https://www.maine.gov/dmr/aquaculture/harvestdata/index.html)).
...
maine_ff <- read_csv("data/maine_finfish.csv") %>%
  ...
  mutate(Species = ifelse(is.na(Species), "Atlantic Salmon", Species),
         Region = "Maine") %>%
  group_by(Species) %>%
  mutate(pounds = zoo::na.locf(pounds))
...
So Connecticut is missing one year for Hard Clam, Massachusetts is missing 2012 and 2013 for Hard Clam and Oysters, Rhode Island is missing 2010 for Oysters and Hard Clam. 
