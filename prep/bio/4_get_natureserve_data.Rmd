...
We need to get scientific names for the data portal species. The Nature Serve R package requires scientific names to query the API.
```{r}
...
dp_sci_names_df_clean <- dp_sci_names_df %>%
  ...
  mutate(n = n()) %>%
  ungroup() %>%
  mutate(sci_fix = case_when( #fixing a couple manually that have multiple scinames AND adding sci name 
