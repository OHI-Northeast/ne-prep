...
library(raster)
library(rgeos)
source('https://raw.githubusercontent.com/OHI-Northeast/ne-prep/blob/gh-pages/src/R/common.R')
library(sf)
...
Using a set of IUCN species range maps, rasterize each species to 1km x 1km raster using `fasterize`.  Use `presence` field from shapefile.
...
maps_to_rasterize <- read_csv("data/iucn_spp_shp_filepaths.csv") %>%
  select(-X1) %>%
    ...
    ####################################################################.
      ...
      tmp <- parallel::mclapply(seq_along(spp_ids),
       ...
       # spp_shp <- valid_check(spp_shp)
        ...
        spp_shp <- spp_shp %>%
          st_transform(crs = "+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs")
