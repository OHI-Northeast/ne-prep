...
svi_2009 <- raw_svi_2009 %>% 
  ...
  select(year, state, city, social_vul, gentrification_vul, fishing_vul, inundation_risk) %>% 
...
svi_overall <- left_join(svi_social, svi_gen, by = c("year", "state")) %>% 
  left_join(svi_inund, by = c("year", "state")) %>% 
  left_join(svi_fish, by = c("year", "state")) %>% 
  select(year, state, res_inund_score,res_fish_score, res_social_score, res_gen_score) %>% 
  rowwise() %>% 
