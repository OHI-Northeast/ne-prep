...
oi_graph <- ggplot(oi_score) +
  geom_line(aes(x=year, y= oi_score, color = state)) +
...
svi_2009 <- raw_svi_2009 %>% 
  ...
  left_join(ma_rgns_by_counties, by = c("city_community", "state")) 
```
...
pick up here for the distiction of north and south mass
  WHY ARE THERE NAS?

```{r}
svi_bind <- rbind(svi_2009, svi_2010, svi_2011, svi_2012, svi_2012, svi_2013, svi_2014, svi_2015, svi_2015, svi_2016) %>% 
  select(-County)
...
svi_overall <- left_join(svi_social, svi_gen, by = c("year", "state","rgn_id", "rgn_name")) %>% 
  left_join(svi_inund, by = c("year", "state", "rgn_id", "rgn_name")) %>% 
  left_join(svi_fish, by = c("year", "state", "rgn_id", "rgn_name")) %>% 
  select(year, state, rgn_id, rgn_name, res_inund_score,res_fish_score, res_social_score, res_gen_score) %>% 
  rowwise() %>% 
  mutate(res_score = mean(c(res_inund_score, res_fish_score, res_social_score, res_gen_score))) %>%
  mutate(rgn_id = case_when(
    ...
    state == "CT" ~ "5",
    state == "NY" ~ "10",
    state == "MA" & rgn_name == "Massachusetts-North" ~ "7",
    state == "MA" & rgn_name == "Massachusetts-South" ~ "8")) %>% 
  select(- state) %>% 
  mutate(rgn_name = case_when(
    rgn_id == "6" ~ "Maine", 
    rgn_id == "9" ~ "New Hampshire", 
...
FIX:
This is all wonky now cause ma is represented twice and he other states dont have their region ids yet. decide if want to keep this
```{r}
svi_social_vul_graph <- ggplot(svi_social) +
  geom_line(aes(x=year, y= score_social, color = state)) +
...
** will need to go back to here and separate north/south ma
although, need to think about how we cant do this for the senate
Combine
```{r}
...
social_resilience_scores <- left_join(lcv_scores, svi_overall, by = c("year", "rgn_id", "rgn_name")) %>% 
  ...
  rowwise() %>% 
  mutate(social_score = mean(c(score_lcv,svi_score,oi_score), na.rm= TRUE))
...
Probably won't use grading the states

Grading the States 

```{r}
raw_gs <- read_excel(file.path(dir_anx, "_raw_data/GradingTheStates/grading_states.xlsx"))
```

```{r}
gs <- raw_gs %>% 
  select(state, overall_grade) %>% 
  rename(rgn_name = state) %>% 
  mutate(raw_score = case_when(overall_grade == "A+" ~ 15,
                           overall_grade == "C" ~ 8,
                           overall_grade == "C+" ~ 9,
                           overall_grade == "D+" ~ 6,
                           overall_grade == "D" ~ 5,
                           overall_grade == "F" ~ 2),
         score = scales::rescale(raw_score, to = c(0,1), from=c(1,15)),
         rgn_id = case_when(
           rgn_name == "Maine" ~ "6", 
...
gs_graph <- ggplot(gs) +
  ...
  ggtitle("Grading the States Score") +
  ylab("Score") +
  xlab("Year") +
  theme_classic()
