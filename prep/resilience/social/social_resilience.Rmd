...
g <- ggplot(hma) +
  geom_line(aes(x=year, y= score, color = dis_num)) +
...
social_resilience_scores <- left_join(lcv_scores, svi_overall, by = c("year", "rgn_id", "rgn_name")) %>% 
  ...
  rowwise() %>% 
  mutate(social_score = mean(c(score_lcv,svi_score,oi_score, bhi_score), na.rm= TRUE)) # can remove NA part when all back filled
