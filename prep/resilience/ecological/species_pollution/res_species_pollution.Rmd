---
title: "OHIEC: Resilience - Species Pollution Resilience prep"
...
4 enteries missing in the nas data, which means that 4 times no species occurances were logged:
RI 2008, 2011, 2012,2013. Even though Rhode Island has a score of 0 for every entry, this is because of very low invasive species occracnes (1= lowest (and the mode) and 7=highest). So here I will give RI a score of 1 because the way that the data is represnted this means that no new invasive species were introduced.
...
write.csv(res_species_pollution, file.path(dir_git,"prep/resilience/ecological/water_pollution/data/res_species_pollution.csv"))
```
...
```{r}
write.csv(res_species_pollution, file.path(dir_calc, "layers/res_species_pollution.csv"))
```
