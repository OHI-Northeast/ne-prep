...
# Data
1. RAM reference from the fisheries goal layer
2. NOAA fisheries assessment data 
3. USGS Protected Areas data set
4. NOAA OLE data pulled from reports
5. NOAA fisheries observer data 
6. Allocated sea days from NOAA fisheries observer programs
...
**4. NOAA OLE**
**Downloaded**: August 8th and 9th 2019

**Description**:  NOAA Office of Law Enforcement Annual Reports for the Northeast Region. Recieved from Casey Brennan casey.brennan@noaa.gov 
...
**4. NOAA Fisheries Observer Data**
**Downloaded**: August 9th 2019

**Description**: Tab one: year, total number of observers, trips, and days at sea. Tab two: year, statistical area, number trips, and number of hauls. Recived data from Debra Duarte debra.duarte@noaa.gov 

**Time range**: 2005-2019

**Format**:  Tabular, excel spreadsheet


**4. Allocated Sea Days information**
**Downloaded**: August 14th 2019

**Description**: Annual with allocated sea days. Used this for target number of seadays https://www.nefsc.noaa.gov/fsb/SBRM/

**Time range**: 2013-2017

**Format**:  pdfs, transfered into to excel spreadsheet
...
Calculate the percent of species landed that have stock assessments, by NEOHI Region (not over time)
```{r}
...
write.csv(percent_assessed_by_region, file = "data/percent_assessed_by_region.csv")
```
...
write.csv(assessment_score, file = "data/assessment_score.csv")
```
...
```{r save_to_layers}
#save marine
write.csv(marine_protected_by_rgn,"data/marine_protected_by_rgn.csv")
```
...
Only have SA data back to 2004 for nmfs, will be harder to determine adequatly assessed in 2005-2008
...
write.csv(percent_adeq_ass, file = "data/percent_adeq_ass.csv")
```
...
###Calculate scores for regulations
```{r}
...
write.csv(fisheries_reg_score, file = "data/fisheries_reg_score.csv")
```

Visualize
```{r}
fisheries_reg_graph <- ggplot(fisheries_reg_score)+
  geom_line(aes(x=year, y= reg_score, color= rgn_name))+
...
write.csv(fish_ob_score, file = "data/fish_ob_score.csv")
```
...
ole_scores <- ole_raw %>% 
  ...
  fill(staffing,.direction = c("down") ) %>%  # a linear regression to gap fill yeilded 11, and a mean would change the downward linear trend. simply using the previous year's data is the most reasonable
  fill(enforcement_action, .direction = c("up")) %>% 
  mutate(compliance_score = 1- (enforcement_action/(inspections+investigations)),
         staff_score = staffing/33, #divide by the maximum of all years
  ...
  rowwise() %>% 
  mutate(enforcement_score_OLE=mean(c(staff_score, num_patrols_score, out_reach_num_score), na.rm=T) )

write.csv(ole_scores, file = "data/ole_scores.csv")
```

```{r}
enforcement_compliance_scores <- ole_score %>% 
  select(year, enforcement_score_OLE, compliance_score) %>% 
  right_join(fish_ob_score, by = c("year")) %>% 
  fill(enforcement_score_obser, .direction = c("up")) %>% #gap fill
  rowwise() %>% 
  mutate(enforcement_score= mean(c(enforcement_score_OLE, enforcement_score_obser), na.rm=T))

write.csv(enforcement_compliance_scores, file = "data/enforcement_compliance_scores.csv")
```
...
```{r}
write.csv(fish_res_score, file.path(dir_calc, "layers/res_fishing.csv"))
```
