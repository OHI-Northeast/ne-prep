...
gen_clean <- raw_gen %>% 
  filter(year >= 2005 & year <=2017) %>% 
  filter(state == "ME" | state == "NH" | state == "MA" | state == "RI" | state == "CT" | state == "NY") %>% 
  filter(energy_source == "Hydroelectric Conventional"| energy_source == "Wind" | energy_source == "Solar Thermal and Photovoltaic" | energy_source == "Pumped Storage" | energy_source == "Nuclear") %>%
  select(year, state, gen_mwh) %>% 
  group_by(year, state) %>% 
  dplyr::summarise(total_power = sum(gen_mwh)) %>% 
...
graph_clean_pro <-ggplot(power_gen)+
  ...
  ggtitle("Percent of Clean/Renewable Energy Production (with nuclear") +
  ylab("Percent") +
  xlab("State") +
  theme_classic() 
graph_clean_pro
...
for(i in 2005:2017){
  ...
  me <- me %>% 
    mutate(target = ( 0.0152*year -30.3592 ))
}
...
for(i in 2005:2017){
  ...
  ma <- ma %>% 
    mutate(target = (0.00692*year - 13.74760))
}
...
for(i in 2005:2017){
  ...
  nh <- nh %>% 
    mutate(target = (-0.0104*year +21.3120)) # they hit their target from the get go so theres this weird negative trend here. this won't matter when calcualte scores because will cap at 1 anyways
}
...
for(i in 2005:2017){
  ...
  ri <- ri %>% 
    mutate(target = (0.0128 *year  -25.6554)) 
}
...
for(i in 2005:2017){
  ...
  ny <- ny %>% 
    mutate(target = (0.0016*year-2.7480 )) 
}
...
for(i in 2005:2017){
  ...
  ct <- ct %>% 
    mutate(target = ( -0.00152*year + 3.52560  )) 
}
...
```{r}
energy_target <- rbind(me, nh, ma, ri, ct, ny)
...
graph_clean_energy_score_nuc <-ggplot(power_gen_scores)+
  geom_line(aes(x=year, y = score, color=state))+
  ggtitle("Clean Energy Score (with nuclear") +
  ylab("Score") +
  xlab("Year") +
  theme_classic() 
graph_clean_energy_score_nuc
ggsave("figs/graph_clean_energy_score_nuc.jpg", width=7, height=5, dpi=300)
...
for(i in 2005:2017){
  ...
  nh_em <- nh_em %>% 
    mutate(target = ( -0.404*year +831.120 ))
}
...
for(i in 2005:2017){
  ...
  ri_em <- ri_em %>% 
    mutate(target = (-0.21*year + 431.82 ))
}
...
for(i in 2005:2017){
  ...
  ct_em <- ct_em %>% 
    mutate(target = (-0.789  *year + 1625.077))
}
...
for(i in 2005:2017){
  ...
  ny_em <- ny_em %>% 
    mutate(target = (-3.76*year + 7758.71))
}
...
Combine all states df with target
```{r}
emissions_target <- rbind(nh_em, ma_em, ri_em, ct_em, ny_em, me_em)
