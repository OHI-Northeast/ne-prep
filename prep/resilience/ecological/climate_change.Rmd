...
me_clean_target <- me %>% 
  rowwise() %>% 
  mutate(target = (0.0152*year_n + 0.2017 ),
...
#coef = 0.0135  ; int = -0.0013 

ma_clean_target <- ma %>% 
  rowwise() %>% 
  mutate(target = (0.0135 *year_n-0.0013 ),
         per_clean = round(per_clean, digits= 2),
         target = round(target, digits= 2)) %>% 
  select(year, state, per_clean, target)
...
#coef=  0.00893 int= 0.06457

nh_clean_target <- nh %>% 
  mutate(target = ( 0.00893*year_n +0.06457),
         per_clean = round(per_clean, digits= 2),
         target = round(target, digits= 2)) %>% 
  select(year, state, per_clean, target)
...
#coef=0.0128  , int = -0.0117 

ri_clean_target <- ri %>% 
   mutate(target = ( 0.0128*year_n -0.0117),
         per_clean = round(per_clean, digits= 2),
         target = round(target, digits= 2)) %>% 
  select(year, state, per_clean, target)
...
#coef=0.0132   , int =  0.1568 

ny_clean_target <- ny %>% 
  mutate(target = (0.0132  *year_n +  0.1568 ),
         per_clean = round(per_clean, digits= 2),
         target = round(target, digits= 2)) %>% 
  select(year, state, per_clean, target)
...
#coef= 0.01703, int=-0.00283 

ct_clean_target <- ct %>% 
  mutate(target = (0.01703*year_n -0.00283),
         per_clean = round(per_clean, digits= 2),
         target = round(target, digits= 2)) %>% 
  select(year, state, per_clean, target)
...
#coe = -0.404  ; int= 21.504

nh_em_target <- nh_em %>% 
  mutate(target = (-0.404 *year_n +21.504),
...
ri_em_target<- ri_em %>% 
    mutate(target = (-0.21*year_n+11.43),
         emission = round(emission, digits= 2),
         target = round(target, digits= 2)) %>% 
  select(year, state, emission, target)
...
#coef=-0.789  ; int = 44.589 

ct_em_target <- ct_em %>% 
  mutate(target = (-0.789*year_n+44.589 ),
         emission = round(emission, digits= 2),
         target = round(target, digits= 2)) %>% 
  select(year, state, emission, target)
...
ny_em_target <- ny_em %>% 
  mutate(target = (-3.76 *year_n+214.76),
         emission = round(emission, digits= 2),
         target = round(target, digits= 2)) %>% 
  select(year, state, emission, target)
