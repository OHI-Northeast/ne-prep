...
**3. EPA Carbon Emissions**
**Downloaded**: August, 2019

**Description**: 'https://www.epa.gov/sites/production/files/2017-09/documents/co2ffc_2015.pdf'

**Time range**: 

**Format**: Tabular

FIX BELOW TO MAKE IT A BLURB ABOUT HOW EACH DATA WAS USED 
FIX EEE BY DOWNLOADING PAST YEARS
FIX THE DOCUMENTATION FOR EEE, REKNIT

##Using the Data
Regulations:
1. America's Pledge Report: 
2. American Council for Energy Efficiency Rankings: https://database.aceee.org/state/

Effectiveness:
1. Take the total CO2 emissions each year and compare to the target, which is calcualted by the state set goals
a. Carbon Emmission Targets:
...
2. Total clean power consumption: https://www.eia.gov/beta/states/states/ny/data/dashboard/total-energy
...
Calculate scores
```{r}
energy_consump_scores <- energy_target %>% 
  mutate(score = per_renew/target,
         score = if_else(score >1, 1, score)) 
#save
write.csv(energy_consump_scores, file = "data/power_gen_scores_nuc")
```

Visualize scores
```{r}  

graph_clean_energy_score_nuc <-ggplot(energy_consump_scores)+
  geom_line(aes(x=year, y = score, color=state))+
...
emissions <- rbind(out1, out2, out3, out4, out5) %>% 
  ...
  mutate(end_target = case_when(state == "Maine" ~ (23.64 - 0.8*(23.64)),
...
delete when done comparing the two
...
graph_clean_energy_score_nuc <-ggplot(power_gen_scores)+
  ...
  theme_classic() 
graph_clean_energy_score_nuc
ggsave("figs/graph_clean_generation_energy_score.jpg", width=7, height=5, dpi=300)
