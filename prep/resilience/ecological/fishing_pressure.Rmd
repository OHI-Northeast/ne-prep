...
**code below very similar to code in 5_assessed_species_lookuptable
```{r}
...
nmfs_catch_sp <- nmfs_catch %>%
  ...
  select(species_low, nmfs_species, nmfs_species_alternate, nmfs_species_alternate_2, year) %>%
...
ram_years <- ram_stock_ass %>%
  select(commonname, areaname, stockid, year) %>%
  distinct() %>%
...
time_series_assessed <- m2 %>%
  ...
  mutate(ass= 1) %>% 
  group_by(stock_long) %>% 
  mutate(assesment_sum_5yr = zoo::rollapply(ass, 5, FUN = sum, align = "right", na.rm = F, partial = T),
         adeq_assessed = if_else(assesment_sum_5yr == 5, 1, 0)) %>% 
  ungroup() 
