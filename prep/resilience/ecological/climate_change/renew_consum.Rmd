...
This script calculates if each region is meeting their annual target necessary to reach their end clean energy consumption goal.This data is used to measure climate change resilience implementation & enforcement and effectiveness & compliance.
...
Did not include the energy imported because there is no way to know what source that came from
```{r load_data}
...
Combine all states energy consumption data into one and calculate the percent of renewable. 
...
Now that we have the actual percentage of energy consumption that comes from renewable, we can perform regressions for each state to determine the annual target needed to meet end target. This is done by creating a state specific df with the start (observed) and end (goal) carbon emissions, and then using the slope and intercept to calculate the annual targets. With this we can calculate if each state is on track to hitting their renewable energy target goal for a given year.
