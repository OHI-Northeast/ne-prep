...
#2015
raw_maine_2012 <- read_csv(file.path(dir_anx, "_raw_data/EPA_ECHO/DMR_Annual/2015/maine/ME_FY2015_NPDES_DMRS.csv"))
...
facilities_inspected <- left_join(num_fac, num_fac_inspec, by = "year_state") %>% 
  separate(year_state, c("year", "state"), sep = "_") %>% 
  mutate(percent_w_inspections = fac_inspection/fac_total,
         year = as.numeric(year))

#save data HOW
#write.csv(facilities_inspected, file = file.path(dir_calc, "prep/resilience/data/facilities_inspected.csv"))
...
figure out how to save the number of rows in a df to do the math? so more reproducible
maybe just use count() without grouping or anything
...
## Maine
maine_2009 <- raw_maine_2009 %>% 
  select(EXTERNAL_PERMIT_NMBR, RNC_DETECTION_CODE) %>% 
  filter(RNC_DETECTION_CODE == "N")

me_09_exp <- count(raw_maine_2009) # expected 85510 reports
me_09_sub <- count(maine_2009) # did not receive 2832
#expected 85510 reports
# did not receive 2832
ME_2009 <- 1- (me_09_sub/me_09_exp) 
#0.9668811 received
...
wp_res_score <- wp_res_metrics %>% 
  mutate(regulation_score = 1) %>% 
  replace(., is.na(.), 0) %>% 
  mutate(target_submitted = if_else(target_submitted == 0, 0.95, target_submitted)) %>% ##next few lines are gap filling for report submission data
  mutate(percent_submitted = if_else(percent_submitted == 0 & state == "CT", 1, percent_submitted)) %>% 
  mutate(percent_submitted = if_else(percent_submitted == 0 & state == "MA", 0.9841761, percent_submitted)) %>% 
  mutate(percent_submitted = if_else(percent_submitted == 0 & state == "ME", 0.9668811, percent_submitted)) %>% 
  mutate(percent_submitted = if_else(percent_submitted == 0 & state == "NH", 0.9971842, percent_submitted)) %>% 
  mutate(percent_submitted = if_else(percent_submitted == 0 & state == "NY", 0.9973308, percent_submitted)) %>% 
  mutate(percent_submitted = if_else(percent_submitted == 0 & state == "RI", 1, percent_submitted)) %>% 
  mutate(inspection_score = percent_w_inspections/target_inspections) %>% 
  mutate(noviolation_score = percent_noviolations/target_noviolations) %>% 
  mutate(submitted_score = percent_submitted/target_submitted) %>% 
  mutate(submitted_score = if_else(submitted_score >1, 1, submitted_score)) %>% # capping the score at 1
  group_by(year, state) %>% 
  mutate(submitted_score = as.numeric(submitted_score)) %>% 
  summarise(average_score = sum(inspection_score, submitted_score, noviolation_score, regulation_score)/4)

#write.csv(wp_res_score, file = "wp_res_score")

graph <- ggplot(wp_res_score) +
  geom_line(aes(x=year, y= average_score, color = state))+
