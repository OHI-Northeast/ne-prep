...
ole <- read_csv(file.path(dir_git, "prep/resilience/ecological/data/ole_scores.csv")) %>% 
  select(year, compliance_score, rgn_id, rgn_name) %>% 
  filter(year >= "2013") #from 2005-2013 jsut back filled in the fisheries layer
...
res_hab_des <- left_join(sob, area_protect, by = c("rgn_id", "rgn_name")) %>% 
 # select(rgn_name, rgn_id, score) %>%
  uncount(5, .id = "n", .remove = F) %>%
  mutate(year = ifelse(n == 1, 2013, n + 2012)) %>%
  select(-n) %>% 
  left_join(ole, c("year", "rgn_id", "rgn_name")) %>% 
  rowwise() %>% 
  mutate(score = mean(c(score_1,score_2, compliance_score), na.rm=T)) 
```
...
Graph
**from before when didn't have ole data so no years
