...
all <- cowplot::plot_grid(inc_g, ag,a2g, bg,cg,dg,eg,fg,gg,
                          labels= c("Areas included in LSP", "Fishery Management Areas","Fishery Management Area", "Closure Area","Shellfish Management Area", "Essential Fish Habitat Conservation Area", "Conservation Area", "Gear Restricted Area", "Special Area Management Plan"),
                          label_size = 8, 
                          ncol = 3)
all
...
lsp_areas <- marine %>% 
  ...
  as_data_frame() %>% 
  select(d_Mang_Typ, d_Mang_Nam, d_Des_Tp, d_State_Nm,Loc_Ds, Unit_Nm ) %>% 
  rename(Management = d_Mang_Typ) %>% 
  rename(Management_Name = d_Mang_Nam) %>% 
  rename(Designation_Type = d_Des_Tp) %>% 
  rename(State = d_State_Nm) %>% 
  rename(Location_Designation = Loc_Ds) %>% 
  rename(Name = Unit_Nm)
