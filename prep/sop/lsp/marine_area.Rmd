...
states <- read_sf(dsn = file.path(dir_prep, "spatial/shapefiles/states.shp")) %>%
  st_transform(us_alb)

## Fishery Management Area
a <- marine  %>% 
  filter(Loc_Ds %in% c("Fishery Management Area", "Fishery Management Areas"))
#mapview(a)
ag <- ggplot(a) +
 geom_sf(states, mapping = aes()) +
  ...
  theme_bw() +
  ggtitle("Fishery Management Areas")
...
cg <- ggplot(c) +
  ...
  geom_sf(states, mapping = aes())+
  theme_bw()+
  ggtitle("Shellfish Management Area")
...
dg <- ggplot(d) +
  ...
  geom_sf(states, mapping = aes())+
  theme_bw()+
  ggtitle("Essential Fish Habitat Conservation Area")
...
gg <- ggplot(g) +
  ...
  geom_sf(states, mapping = aes())+
  theme_bw()+
  ggtitle("Special Area Management Plan")

all <- cowplot::plot_grid(ag,bg,cg,dg,eg,fg,gg)
all
```
