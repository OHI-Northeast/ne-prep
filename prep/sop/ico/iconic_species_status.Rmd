...
```{r}
update_df <- iconic_species_status %>%
  ...
  mutate(final_score = case_when(
    count == 1 ~ sum(emily, natureserve, na.rm = T),
...
spp_rgns_scores <- bind_rows(spp_rgns, update_df) %>%
  ...
  mutate(count = n()) %>%
  ungroup() %>%
  mutate(drop = ifelse(is.na(score) & count == 2, 1, 0)) %>%
  filter(drop == 0) %>%
  select(-count, -drop)
...
For those species in a region where they are found but without a state status, use the USA designated one
