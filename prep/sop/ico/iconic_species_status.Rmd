...
ggplotly(ico_heatmap_score)

ggsave("figs/ico_heatmap_score.jpg", width=7, height=5, dpi=300)


```


graph heatmap with status label
```{r}

ico_heatmap <- ggplot(data = table_status, aes(x = rgn_name, y = common)) +
 #geom_tile(fill ='grey90')  +
  geom_tile(aes(fill = Status, alpha = present ))+ #alpha = present 
