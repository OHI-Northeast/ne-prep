...
mclapply(files, get_wetland_cells, mc.cores = 12)
```
...
states <- c("/nh", "/ny", "/me", "/ri", "/ct") #added the slash because "me" was bringing all rasters
...
```{r visualize}
ggplot(full_df, aes(x = year, y = total_loss, color = state)) +
  geom_line() +
  theme_bw() +
  labs(title = "Change in salt marsh habitat",
       x = "Year",
       y = "Habitat loss (%) since 1770-1850s")
```
...
toolbox_layer <- full_df %>%
  ...
  mutate(score = na.approx(total_loss)) %>%
  complete(year = 1996:2017) %>%
  mutate(score = na.locf(score)) %>% #now we carry forward the 2010 value for all years to 2017 %>%
  ungroup() %>%
  mutate(score = ifelse(score <= 0, 100, 100-score)) %>% #this keeps the score from going above 100 for Maine
  left_join(rgns) %>%
  select(rgn_id, rgn_name, year, score)

write_csv(toolbox_layer, file.path(dir_calc, "layers/hab_salt_marsh.csv"))

ggplot(toolbox_layer, aes(x = year, y = score, color = rgn_name)) +
  geom_line() +
  theme_bw() +
  labs(x = "Year",
       y = "Score",
