...
Look at lobster catch from DMIS database
...
lob_dmis_map <- stat_shp %>%
  filter(Id %in% ohi_stat_areas$Id) %>%
  left_join(clean_dmis, by = c("Id" = "stat_area")) %>%
  select(Id, year, lbs_landed)

ggplot(lob_dmis_map) +
  geom_sf(aes(fill = lbs_landed)) +
  facet_wrap(~year) +
  theme_bw()
...
dmis_lob_map_ohi <- calc_prop_area %>%
  left_join(clean_dmis, by = c("Id" = "stat_area")) %>%
  mutate(catch = ohi_rgn_prop_area*lbs_landed) %>%
  group_by(rgn_id, rgn_name, year) %>%
  summarize(catch = sum(catch)) %>%
  select(rgn_name, rgn_id, year, catch)


ggplot(dmis_lob_map_ohi) +
  geom_sf(aes(fill = catch), size = 0.1) +
  theme_bw() +
  facet_wrap(~year)
```
