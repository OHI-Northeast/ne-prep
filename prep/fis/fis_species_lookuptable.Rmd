---
title: "Create a species information lookup table"
...
# Summary
This script creates a fish lookup table to match species from NMFS catch data, RAM database and NMFS stock assessment data. The stock assessment species are not identical to the species listed in the catch data so we need to make sure they match.
...
Now lets just get all OHI regions where the remaining stocks are caught. The easier alternative to this would be to assign all OHI regions to all of these stocks, but if this data is used outside of scores that would possible indicate stocks existing where they do not.
...
```{r}
write_csv(comp_dataset, "data/species_lookup_table_catch_stock_assessments.csv")
```
