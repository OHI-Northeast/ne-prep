...
We need to **gapfill** this missing data. When a species/state combination has missing data for a year, we can not assume it has a catch of 0. This is a large assumption, but without better information that indicates catches exist for the missing years, and what those values are, we feel this is the best option to address missing data. We also calculate a rolling average of catch so NAs will cause those averages to rely on just one or two years of catch. This is done to account for any wild fluctuations in catch year to year.
