...
Link with noaa catch data and save for data exploration and dashboard

```{r}
ram_noaa <- stock_scores %>%
  mutate(species = toupper(common_noaa)) %>%
  left_join(noaa) %>%
  filter(year > 1979)

write.csv(ram_noaa, file = "data/noaa_catch_and_ram_data.csv")
```

Clean up layers to save for toolbox. This means adding region ids and selecting specific columns necessary

```{r}
catch <- ram_noaa %>%
  select(common_noaa, year, state, pounds) %>%
  distinct() %>%
  group_by(state, common_noaa) %>%
  complete(year = 1980:2017) %>%
  mutate(catch = ifelse(is.na(pounds), 0, pounds),
         mean_catch = zoo::rollmean(catch, 3, fill = NA)) %>%
  filter(year > 2005) %>%
  select(year, common_noaa, state, mean_catch)

write.csv(catch, file = file.path(dir_calc, "layers/fis_meancatch.csv"))
```

## NOTE: I am gapfilling years with no catch data with a 0 instead of NA. confirm with NOAA


Now we can score. *Think about moving this to functions.R*
...
scores_weight <- ram_noaa %>%
    group_by(year, state) %>%
    mutate(SumCatch = sum(pounds)) %>%
    ungroup() %>%
    mutate(wprop = pounds / SumCatch) %>%
  distinct()

state_status <- scores_weight %>%
    group_by(state, year) %>%
    summarize(status = sum(score * wprop)) %>%
    ungroup() %>%
  filter(!is.na(state))

ggplot(state_status %>% filter(year > 2000), aes(x = year, y = status*100, color = state)) +
  geom_line() +
  theme_bw() +
  labs(x = "Year",
       y = "Fisheries Status")
...
ggplot(stock_plot_df, aes(x = year, y = score)) +
  geom_line(data = state_status %>% filter(year > 2000), aes(x = year, y = status), color = "black") +
    theme_bw() +
    geom_line(aes(group = common_noaa, color = common_noaa,
