...
ss_toolbox <- nmfs_ss %>%
  ...
  select(year, source, nmfs_original_species, location, score, rgn_id)
```
...
catch_ss <- ss_toolbox %>%
  left_join(catch, by = c("year", "rgn_id", "nmfs_original_species" = "species")) %>%
  filter(!is.na(mean_catch)) %>%
  group_by(year, rgn_id) %>%
  mutate(yr_rgn_catch = sum(mean_catch_times_prop),
         catch_prop = mean_catch_times_prop/yr_rgn_catch) %>%
  ungroup() %>%
  mutate(display_name = ifelse(is.na(stock_id), nmfs_original_species, stock_id))
```

Lines are weighted based on the catch_prop

```{r}
ggplot(catch_ss %>% filter(catch_prop > 0.1), aes(x = year, y = score, color = display_name, size=catch_prop)) +
  geom_line() +
  theme_bw() +
  facet_wrap(~rgn_id) +
