...
```{r}
me_2016_soc <- st_read(dsn=file.path(dir_anx, "_raw_data/NOAA_ESI/ME_NH_2016_GDB/Maine_NewHampshire_2016_ESI.gdb"), layer = "SOCECON_POINT") %>%
  st_transform(us_alb) %>%
...
```{r}
me_2016_nav <- st_read(dsn=file.path(dir_anx, "_raw_data/NOAA_ESI/ME_NH_2016_GDB/Maine_NewHampshire_2016_ESI.gdb"), layer = "NAV_MARINE_POINT") %>%
  st_transform(us_alb) %>%
  st_intersection(me_shape) %>%
  mutate(TYPE = as.character(TYPE)) %>%
...
```{r}
me_2016_mgd <- st_read(dsn=file.path(dir_anx, "_raw_data/NOAA_ESI/ME_NH_2016_GDB/Maine_NewHampshire_2016_ESI.gdb"), layer = "MANAGED_POINT") %>%
  st_transform(us_alb) %>%
  st_intersection(me_shape) %>%
  mutate(TYPE = as.character(TYPE)) %>%
  filter(TYPE == "NL") %>%
