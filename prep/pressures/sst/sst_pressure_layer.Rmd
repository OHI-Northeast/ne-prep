...
By extracting the data for each of the 11 regions using the `zonal` function from the `raster` package we can get the mean score per region.
...
region_stats  <- zonal(pressure_stack,  zones, fun="mean", na.rm=TRUE, progress="text") %>%
                  data.frame()

#extract data for entire region (region 12)
rgn_12_zones <- raster(file.path(dir_git, "spatial/ocean_rasters/rgn_12_zone.tif"))
ne_stats <- zonal(pressure_stack,  rgn_12_zones, fun="mean", na.rm=TRUE, progress="text") %>%
                  data.frame()

regions_stats <- bind_rows(region_stats, ne_stats)
...
data           <- merge(rgn_data, regions_stats, all.y=TRUE, by.x="rgn_id", by.y="zone") %>%
                    dplyr::select(-area_km2, -state_abv, -state_name, -state)%>%
