...
Don't need to download the data again, because it's already been downloaded for the most recent year of global, so just grab the NetCDF files from the global folder.
```{r read_files}
...
}, movie.name = 'slr_annual_rescaled_jv.gif')
```
...
write.csv(slr_data, file.path("out/slr_JV.csv"), row.names=FALSE)
```
...
print(Motion, file='figs/slr_jv.html')
```
