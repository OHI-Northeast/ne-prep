...
animation::saveGIF({
  for(i in 1:nlayers(catch_rasts)){
    ...
    plot(catch_rasts[[i]], zlim=c(0,140000), axes=F, col=cols, add=T)
...
We will use the 99.99th quantile. The majority of catch is below 20,000 tons/km2. But in 2009, there is a record of 138,371 tons/km2. This seems like an outlier so to rescale, we will use the 99.99th quantile instead.
...
catch_ref  <- quantile(all_vals, na.rm = T, probs = 0.9999)
```
...
catch_z     <- zonal(catch_stack, zones) %>% #zonal's default statistic is to take the mean
  as.data.frame() %>%
...
```{r}
write_csv(catch_z, "~/github/ne-scores/region/layers/prs_fish.csv")
```
