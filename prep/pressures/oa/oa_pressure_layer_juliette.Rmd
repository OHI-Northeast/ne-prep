...
#code for looking at the NetCDF files directly
...
foreach(file = nc_files) %dopar% {
  ...
  r <- raster(file) %>%
    ...
    mask(three_nm, filename = sprintf("%s/prs_oa/int/noaa/oa_annual_noaa_%s.tif", 
                                dir_anx, yr), overwrite = TRUE)

  ## write raster to int folder in prs_slr
  fp <- sprintf("%s/int/noaa/oa_annual_noaa_%s.tif", file.path(dir_anx, "prs_oa"), yr)
