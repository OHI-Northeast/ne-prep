...
Using the threshold of 1.5. This is when mild dissolution of pteropods start to occur. 1.2 is severe, we could use this instead.
 ...
 # file = files[1]
  ...
  yr <- str_sub(file, -8,-5)
   ...
   rescale <- raster::raster(file) %>%
    calc(fun = function(x){ifelse(x <=1.5, 1.5, x)}) %>% # 1 is our arg threshold; and also the highest pressure because thats when things start going wonky
    calc(fun = function(x){ifelse(x >= ref, 0, (1.5-x/ref))})  # set equal to one if greater than ref, otherwise scale
