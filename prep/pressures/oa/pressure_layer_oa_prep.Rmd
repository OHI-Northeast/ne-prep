...
I am having issues wrapping my head around this and rescaling because the max reference point is unclear. the minimum is 1. i am looking in the literature for maximum aragonite values in the northeast, or we can always find the max values of all our rasters

The maximum aragonite saturation values by year:
 ...
 # file = files[1]
  ...
  yr <- str_sub(file, -8,-5)
   ...
   rescale <- raster::raster(file) %>%
    calc(fun = function(x){ifelse(x <=1, 1, x)}) %>% # 1 is our arg threshold; and also the highest pressure because thats when things start going wonky
    calc(fun = function(x){ifelse(x >= ref, 0, (1-x/ref))})  # set equal to one if greater than ref, otherwise scale
    
 # rescale <- (1-rescale) # the math above should handle this    
