...
- 1 = a pressure of 1. This is a widely accepted saturation lethal threshold for pteropods

- 1.5 = a pressure of 0.8. This saturation level is when mild dissolution begins to occurr 
(source: https://www.frontiersin.org/articles/10.3389/fmars.2019.00227/full)

- 2.5 = a pressure of 0. This is the lowest saturation level where few organims are affected source(https://pubs.geoscienceworld.org/gsa/geology/article/37/12/1131/103987/Marine-calcifiers-exhibit-mixed-responses-to-CO2)
 ...
 # file = files[1]
  ...
  yr <- str_sub(file, -8,-5)
   ...
   rescale <- raster::raster(file) %>%
    ...
    calc(fun = function(x){ifelse(x >=1 & x <= ref_1, -0.4*x+1.4, x)}) %>% 
    calc(fun = function(x){ifelse(x >= ref_1 & x <= ref_2, -0.8*x + 2, x)}) %>% 
