...
Read in the annual filed from global, will need to do the prep for it if we end up using this data instead of the newer data

```{r}
files <- list.files(file.path(dir_M, 'git-annex/globalprep/prs_oa/v2017/int/annual_avg_moll'), full.names=TRUE)
files <- files[-60, -63, -63] #theres a weird extra file here and also we don't want 2020 on on estimates
...
###Results: create a gif using the global data 1958-2017
```{r gif_results}
library(animation)
...
###Results: create a gif using the global data 2014-2017
```{r gif_results}
library(animation)
