...
```{r calculate_score}
aqua_hab_prs <- left_join(production, sf_watch, by = ("Species")) %>% 
...
write.csv(aqua_hab_prs, file = "data/aqua_hab_prs.csv")
```


Graph
```{r graph}
aqua_hab_prs_graph <-ggplot(aqua_hab_prs)+
  geom_line(aes(x=year, y = prs_score, color=rgn_id))+
  ggtitle("Aquaculture habitat pressure score") +
  ylab("Score") +
  xlab("Year") +
  theme_classic() 
aqua_hab_prs_graph

ggsave(file.path(dir_git, "prep/pressures/habitat/figs/aqua_hab_prs_graph.png"), width=7, height=5, dpi=300)
```
