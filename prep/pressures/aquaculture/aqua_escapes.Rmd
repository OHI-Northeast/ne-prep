...
```{r}
aqu_escape_prs <- escape_pro %>% 
  group_by(Year, Region) %>% 
  dplyr::summarize(score = mean(scaled_score, na.rm= TRUE)) %>% 
...
```{r visualize}  
aqu_esc_graph <-ggplot(aqu_escape_prs)+
  geom_line(aes(x=Year, y = score, color=Region))+
...
ggsave(file.path(dir_git, "prep/pressures/aquaculture/aqu_esc_graph.png"), width=7, height=5, dpi=300)
```
