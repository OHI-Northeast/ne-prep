...
disease_prs_int <- left_join(production, sf_watch, by = ("Species")) %>% 
...
## we calculated the reference point in 2019 as 2*max(disease_prs_int$scaled_area_prod) = 21871 to give the pressure some room to grow. We will manually input it here instead of recalcualting each year, resulting in a moving reference point where the highest prodcution year will likely be the most recent
```
...
production_rgn12 <- production %>% 
  ...
  summarise(total_production = sum(production, is.na = TRUE)) %>% 
  ungroup() %>% 
  mutate(rgn_id = 12,
         rgn_name = "Northeast")
...
disease_prs_rgn12 <- left_join(production_rgn12, sf_watch, by = ("Species")) %>% 
  rowwise() %>% 
  mutate(sfw_score = AqCriteria7/10,
         scaled_production = (total_production*sfw_score)) %>% 
...
## we calculated the reference point in 2019 as 2*max(disease_prs_rgn12$scaled_area_prod) = 6608 to give the pressure some room to grow. We will manually input it here instead of recalcualting each year, resulting in a moving reference point where the highest production year will likely be the most recent
```

Combine rgn12 to rest
```{r}
disease_prs <- disease_prs_int %>% 
  rbind(disease_prs_rgn12)
```
