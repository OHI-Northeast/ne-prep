...
Read in the state waters shapefiles and grab the total area
```{r}
...
escape_prs <- left_join(production, sf_watch, by = ("Species")) %>% 
  ...
  mutate(sfw_score = AqCriteria6/10,
         scaled_production = (production*sfw_score)/Area_km) %>% 
  group_by(year, rgn_name, rgn_id) %>% 
  summarise(sum_scale_prod = sum(scaled_production, na.rm = TRUE)) %>% 
  ungroup() %>% 
  #group_by(rgn_name) %>% 
  mutate(ref = 2*max(sum_scale_prod, na.rm = TRUE)) %>% 
  ungroup() %>% 
  rowwise() %>% 
  mutate(prs_score = sum_scale_prod/ref)
```
