...
escape_prs_int <- left_join(production, sf_watch, by = ("Species")) %>% 
...
## we calculated the reference point in 2019 as 2*max(escape_prs$scaled_area_prod, na.rm = TRUE) = 14575 to give the pressure some room to grow. We will manually input it here instead of recalcualting each year, resulting in a moving reference point where the highest prodcution year will likely be the most recent
```
...
## we calculated the reference point in 2019 as 2*max(disease_prs_rgn12$scaled_area_prod, na.rm = TRUE) = 3982 to give the pressure some room to grow. We will manually input it here instead of recalcualting each year, resulting in a moving reference point where the highest production year will likely be the most recent
```
