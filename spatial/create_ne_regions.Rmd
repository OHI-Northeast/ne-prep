...
ma_region_south <- st_difference(ma_region_2, north_ma_mask) %>%
  ...
  mutate(rgn_name = "Massachusetts-Virginian")
ma_region_north <- st_difference(ma, ma_region_south) %>%
  select(STUSPS10, NAME10) %>%
  mutate(rgn_name = "Massachusetts-Gulf of Maine")
...
ne_ohi_rgns <- clean_epu %>%
  ...
  mutate(rgn_id = case_when(
    ...
    rgn_name == "Maine" ~ 6,
    rgn_name == "Massachusetts-Gulf of Maine" ~ 7,
    rgn_name == "Massachusetts-Virginian" ~ 8,
